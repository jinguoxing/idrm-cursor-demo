...
| BR-24 | 主键策略 | 所有数据表主键使用 UUID v7 格式（36字符），支持分布式环境下的唯一性保证和时间排序，使用 github.com/google/uuid 库生成 |
...
|-------|-------------|-------------|
| 用户ID | 用户唯一标识 | 系统生成，UUID v7 格式（36字符），唯一，不可修改，支持分布式环境下的唯一性保证和时间排序 |
...
| JWT Token | 认证令牌 | 包含用户ID（UUID v7格式）、过期时间等信息 |
| Token过期时间 | Token的过期时间戳 | 7天有效期 |
| 登录历史ID | 登录记录唯一标识 | 系统生成，UUID v7 格式（36字符），唯一，不可修改 |
| 登录用户ID | 登录的用户ID | 关联用户表，UUID v7 格式（36字符），外键关联 users.id |
